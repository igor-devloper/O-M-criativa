...
export function ChecklistSection({ maintenanceId, checklistItems, isCompleted }: ChecklistSectionProps) {
  ...
  const saveChecklist = async () => {
    setIsSaving(true)
    try {
      const response = await fetch(`/api/manutencao/${maintenanceId}/checklist`, {
        method: "POST",
        headers: {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Itens a serem verificados durante a manutenção</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
                ...
                <div className="grid gap-1.5 leading-none">
                  ...
                  {item.completedAt && (
                    <p className="text-xs text-muted-foreground">
                      Concluído em: {format(new Date(item.completedAt), "PPP", { locale: ptBR })}
                    </p>
                  )}
                </div>
              </div>
          ...
          {!isCompleted && (
            <Button onClick={saveChecklist} className="w-full mt-4" disabled={isSaving}>
